...
async def db_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    async with test_engine.begin() as conn:
        # Drop storage tables (foreign keys first)
...
def sample_file_data(test_user_id: UUID) -> Dict[str, Any]:
    ...
    return {
        ...
        "filename": f"{uuid4()}_test_image.jpg",
...
async def sample_file(
    ...
    return await create_file_in_db(
        db_session,
        owner_id=test_user_id,
        filename=f"{uuid4()}_test_image.jpg",
        original_filename="test_image.jpg",
...
async def sample_file_with_thumbnails(
    ...
    file = await create_file_in_db(
        db_session,
        owner_id=test_user_id,
        filename=f"{uuid4()}_image_with_thumbs.jpg",
        original_filename="image_with_thumbs.jpg",
        content_type="image/jpeg",
        size_bytes=1024 * 200,  # 200 KB
        storage_backend="local",
        checksum="thumb123hash456",
        is_public=False,
        description="An image with thumbnails",
    )

    # Create thumbnails
    thumbnails = []
    for size, (width, height) in [("small", (100, 100)), ("medium", (300, 300)), ("large", (600, 600))]:
        thumb = await create_file_thumbnail_in_db(
            db_session,
            file_id=file.id,
            size=size,
            width=width,
...
async def sample_files(
    ...
    files_data = [
        ...
        {
            "filename": f"{uuid4()}_image4.webp",
            "original_filename": "modern.webp",
...
async def sample_storage_quota_near_limit(
    ...
    """Create a sample storage quota near its limit."""
    quota_owner_id = uuid4()  # Use different owner to avoid conflict
    return await create_storage_quota_in_db(
        db_session,
        owner_id=quota_owner_id,
        quota_bytes=104857600,  # 100 MB
        used_bytes=104457600,  # ~99.6 MB (~400KB remaining)
        file_count=50,
    )
...
async def sample_empty_storage_quota(
    ...
    """Create a sample empty storage quota."""
    quota_owner_id = uuid4()  # Use different owner to avoid conflict
    return await create_storage_quota_in_db(
        db_session,
        owner_id=quota_owner_id,
...
def sample_thumbnail_sizes() -> Dict[str, Dict[str, int]]:
    """Return standard thumbnail size configurations."""
    return {
...
def image_content_types() -> List[str]:
    """Return list of supported image content types."""
    return [
        "image/jpeg",
        "image/png",
        "image/gif",
...
def allowed_content_types() -> List[str]:
    """Return list of all allowed content types for upload."""
    return [
        "image/jpeg",
        "image/png",
        "image/gif",
