/**
 * LAYA Teacher App - Secure Storage Tests
 *
...
describe('SecureStorage', () => {
  ...
  describe('STORAGE_KEYS', () => {
    it('should have correct predefined storage keys', () => {
      expect(STORAGE_KEYS.ACCESS_TOKEN).toBe('laya_teacher_secure_access_token');
      expect(STORAGE_KEYS.REFRESH_TOKEN).toBe('laya_teacher_secure_refresh_token');
      expect(STORAGE_KEYS.USER_ID).toBe('laya_teacher_secure_user_id');
      expect(STORAGE_KEYS.USER_EMAIL).toBe('laya_teacher_secure_user_email');
      expect(STORAGE_KEYS.BIOMETRIC_ENABLED).toBe('laya_teacher_secure_biometric_enabled');
      expect(STORAGE_KEYS.LAST_LOGIN_AT).toBe('laya_teacher_secure_last_login_at');
    });

    it('should have namespace prefix for all keys', () => {
      const allKeys = Object.values(STORAGE_KEYS);
      allKeys.forEach((key) => {
        expect(key).toMatch(/^laya_teacher_secure_/);
      });
    });
  });
