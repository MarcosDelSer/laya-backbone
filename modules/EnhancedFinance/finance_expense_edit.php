...
/**
 * Enhanced Finance Module - Expense Edit
 *
 * Form for editing existing expenses. Allows modifying category, vendor,
 * amounts, dates, payment method, and notes.
 *
...
if (isActionAccessible($guid, $connection2, '/modules/EnhancedFinance/finance_expense_edit.php') == false) {
    ...
    if ($expense['status'] == 'Rejected') {
        $page->addError(__('This expense has been rejected and cannot be edited.'));
        return;
    }
    ...
    $page->return->addReturns([
        ...
        'success1' => __('Expense updated successfully.'),
        'error1' => __('There was an error updating the expense.'),
    ...
    // Description
    echo '<p>';
    echo __('Use this form to edit the expense. You can modify the category, amounts, vendor, and add any relevant notes. Tax can be applied automatically based on module settings.');
    echo '</p>';
    ...
    // Create form
    $form = Form::create('expenseEdit', $session->get('absoluteURL') . '/modules/EnhancedFinance/finance_expense_editProcess.php');
    $form->setFactory(DatabaseFormFactory::create($pdo));

    $form->addHiddenValue('address', $session->get('address'));
    $form->addHiddenValue('gibbonEnhancedFinanceExpenseID', $gibbonEnhancedFinanceExpenseID);
    ...
    $row = $form->addRow();
        ...
        $row->addSelect('gibbonEnhancedFinanceExpenseCategoryID')
            ...
            ->selected($expense['gibbonEnhancedFinanceExpenseCategoryID'])
            ->required();
    ...
    $row = $form->addRow();
        ...
        $row->addDate('expenseDate')
            ->setValue(Format::date($expense['expenseDate']))
            ->required();
    ...
    $row = $form->addRow();
        ...
        $row->addSelect('paymentMethod')
            ...
            ->selected($expense['paymentMethod'])
            ->required();
    ...
    $row = $form->addRow();
        ...
        $row->addCurrency('amount')
            ->setID('amount')
            ->placeholder('0.00')
            ->setValue(number_format((float)$expense['amount'], 2, '.', ''))
            ->required();
    ...
    $row = $form->addRow();
        ...
        $row->addCurrency('taxAmount')
            ->setID('taxAmount')
            ->placeholder('0.00')
            ->setValue(number_format((float)$expense['taxAmount'], 2, '.', ''));
    ...
    $row = $form->addRow();
        ...
        $row->addCurrency('totalAmount')
            ->setID('totalAmount')
            ->placeholder('0.00')
            ->setValue(number_format((float)$expense['totalAmount'], 2, '.', ''))
            ->readonly();
    ...
    // Receipt Section
    $form->addRow()->addHeading(__('Receipt'));

    // Current receipt (if any)
    if (!empty($expense['receiptPath'])) {
        $receiptURL = $session->get('absoluteURL') . '/' . $expense['receiptPath'];
        $row = $form->addRow();
            $row->addLabel('currentReceipt', __('Current Receipt'));
            $row->addContent('<a href="' . htmlspecialchars($receiptURL) . '" target="_blank" class="text-blue-600 hover:underline">' . __('View Current Receipt') . '</a>');

        // Option to remove receipt
        $row = $form->addRow();
            $row->addLabel('removeReceipt', __('Remove Receipt'))
                ->description(__('Check to remove the current receipt.'));
            $row->addCheckbox('removeReceipt')
                ->checked(false);
    }

    // Upload new receipt
    $row = $form->addRow();
        $row->addLabel('receiptFile', __('Upload Receipt'))
            ->description(__('Upload a new receipt (image or PDF).'));
        $row->addFileUpload('receiptFile')
